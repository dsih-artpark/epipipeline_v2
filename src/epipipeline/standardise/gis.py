...
def dist_mapping(*, stateID: str, districtName: str, df: pd.DataFrame, threshold: int = 65) -> Tuple[str, str]:
    ...
    districtName = re.sub(r"\s\(?\s?U\)?$", " Urban", districtName, flags=re.IGNORECASE)
    districtName = re.sub(r"\s\(?\s?R\)?$", " Rural", districtName, flags=re.IGNORECASE)
    ...
    districtName = re.sub(r"B[ae]ngal[ou]r[ue]\s?City|Bbmp", "Bengaluru Urban", districtName, flags=re.IGNORECASE)
    districtName = re.sub(f"Balasore", "Baleshwar", districtName, flags=re.IGNORECASE)
